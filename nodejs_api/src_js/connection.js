...
const GorgonzolaNative = require("./gorgonzola_native.js");
...
class Connection {
   ...
   *
   * @param {gorgonzola.Database} database the database object to connect to.
    ...
    if (!this._isInitialized) {
      if (!this._initPromise) {
        if (!this._connection) {
          ...
          this._connection = new GorgonzolaNative.NodeConnection(database);
        }
   ...
   * @returns {GorgonzolaNative.NodeConnection} the underlying native connection.
   ...
   * @param {gorgonzola.PreparedStatement} preparedStatement the prepared statement to execute.
   ...
   * @returns {Promise<gorgonzola.QueryResult>} a promise that resolves to the query result. The promise is rejected if there is an error.
   */
  execute(preparedStatement, params = {}, progressCallback) {
    return new Promise((resolve, reject) => {
      ...
      this._getConnection()
        .then((connection) => {
          const nodeQueryResult = new GorgonzolaNative.NodeQueryResult();
          try {
   ...
   * @param {gorgonzola.PreparedStatement} preparedStatement the prepared statement
   * @param {Object} params a plain object mapping parameter names to values.
   * @returns {Array<gorgonzola.QueryResult> | gorgonzola.QueryResult} an array of query results. If there is only one query result, the function returns the query result directly.
   ...
   * @returns {Promise<gorgonzola.PreparedStatement>} a promise that resolves to the prepared statement. The promise is rejected if there is an error.
   */
  prepare(statement) {
    return new Promise((resolve, reject) => {
      ...
      this._getConnection()
        .then((connection) => {
          const preparedStatement = new GorgonzolaNative.NodePreparedStatement(
            connection,
            statement
          );
   ...
   * @param {String} statement the statement to prepare. 
   * @returns {gorgonzola.PreparedStatement} the prepared statement.
   * @throws {Error} if there is an error.
   */
   ...
   * @param {GorgonzolaNative.NodeQueryResult} nodeQueryResult the current node query result.
   * @returns {Promise<gorgonzola.QueryResult>} a promise that resolves to the next query result. The promise is rejected if there is an error.
   */
  _getNextQueryResult(nodeQueryResult) {
    return new Promise((resolve, reject) => {
      const nextNodeQueryResult = new GorgonzolaNative.NodeQueryResult();
   ...
   * @param {GorgonzolaNative.NodeQueryResult} nodeQueryResult the node query result.
   * @returns {Promise<Array<gorgonzola.QueryResult>> | gorgonzola.QueryResult} a promise that resolves to an array of query results. The promise is rejected if there is an error.
   */
   ...
   * @param {GorgonzolaNative.NodeQueryResult} nodeQueryResult the node query result.
   * @returns {Array<gorgonzola.QueryResult> | gorgonzola.QueryResult} an array of query results.
   * @throws {Error} if there is an error.
   */

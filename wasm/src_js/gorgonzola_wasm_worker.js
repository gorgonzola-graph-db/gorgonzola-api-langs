/**
 * @file gorgonzola_wasm_worker.js is the file for the worker thread that wraps around 
...
const objectsStore = {};
let FS = null;
const gorgonzolaSync = require('./sync');
...
if (!isWorkerRuntime) {
  ...
  expose({
    async init() {
      try {
        await gorgonzolaSync.init();
        FS = gorgonzolaSync.getFS();
    ...
    getVersion() {
      return gorgonzolaSync.getVersion();
    },

    getStorageVersion() {
      return gorgonzolaSync.getStorageVersion();
    },
    ...
    databaseConstruct(databasePath,
      ...
      const id = uuidv4();
      try {
        objectsStore[id] = new gorgonzolaSync.Database(
          databasePath,
          bufferPoolSize,
          maxNumThreads,
          enableCompression,
    ...
    connectionConstruct(databaseId, numThreads = null) {
      ...
      const id = uuidv4();
      try {
        objectsStore[id] = new gorgonzolaSync.Connection(

...
///
/// ```
/// # use gorgonzola::{Connection, Database, SystemConfig, Value, Error};
...
impl<'a> Connection<'a> {
    ...
    ///
    /// # Arguments
    /// * `query`: The query to prepare. See <https://gorgonzoladb.com/docs/cypher> for details on the
    ///   query format.
    ...
    ///
    /// # Arguments
    /// * `query`: The query to execute. See <https://gorgonzoladb.com/docs/cypher> for details on the
    ///   query format.
    // TODO(bmwinger): Instead of having a Value enum in the results, perhaps QueryResult, and thus query
    // should be generic.
    //
    // E.g.
    // let result: QueryResult<gorgonzola::value::List<gorgonzola::value::String>> = conn.query("...")?;
    // let result: QueryResult<gorgonzola::value::Int64> = conn.query("...")?;
    //
    ...
    ///```
    /// # use gorgonzola::{Database, SystemConfig, Connection, Value};
    /// # use anyhow::Error;
    /// #
        ...
        $(
            ...
            fn $name() -> Result<()> {
                ...
                let directory: String = if cfg!(windows) {
                    std::env::var("GORGONZOLA_LOCAL_EXTENSIONS")?.replace("\\", "/")
                } else {
                    std::env::var("GORGONZOLA_LOCAL_EXTENSIONS")?
                };
                let name = stringify!($name);
                conn.query(&format!("LOAD EXTENSION '{directory}/{name}/build/lib{name}.gorgonzola_extension'"))?;
                Ok(())
            }
        )*
        }

...
/// The Database class is the main class of `GorgonzolaDB`. It manages all database components.
pub struct Database {
...
pub struct SystemConfig {
    ...
    /// If true, any WAL replaying failure when loading the database will throw an error. Otherwise, Gorgonzola will silently ignore the failure and replay up to where the error occured.
...
mod tests {
    ...
    fn test_database_in_memory() -> Result<()> {
        ...
        // If the special name is ever changed (or removed) gorgonzola is likely to just create a db directory with that name

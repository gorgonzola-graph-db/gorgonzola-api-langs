...
EXTENSION_CMAKE_PREFIX = 'add_definitions(-DGORGONZOLA_EXTENSION_VERSION="'
...
def test_extension_install_httpfs(conn_db_readwrite: ConnDB, tmpdir: str, extension_extension_dir_prefix: str) -> None:
    ...
    extension_path = (
        ...
        .joinpath(
            ...
            extension_extension_dir_prefix,
            "httpfs",
            "libhttpfs.gorgonzola_extension",
        )
        .resolve()
    )
    opener = urllib.request.build_opener()
    opener.addheaders = [("User-agent", "Gorgonzola Test Suite")]
    urllib.request.install_opener(opener)
    download_url = f"http://extension.gorgonzoladb.com/v{extension_version}/{extension_extension_dir_prefix}/httpfs/libhttpfs.gorgonzola_extension"
    temp_path = Path(tmpdir) / "libhttpfs.gorgonzola_extension"

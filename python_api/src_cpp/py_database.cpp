...
PyDatabase::PyDatabase(const std::string& databasePath, uint64_t bufferPoolSize,
    ...
    gorgonzola::extension::ExtensionUtils::addTableFunc<gorgonzola::PandasScanFunction>(*database);
    storageDriver = std::make_unique<StorageDriver>(database.get());
    py::gil_scoped_acquire acquire;
    if (gorgonzola::importCache.get() == nullptr) {
        gorgonzola::importCache = std::make_shared<gorgonzola::PythonCachedImport>();
    }
}

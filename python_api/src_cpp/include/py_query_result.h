...
public:
    ...
    static py::object convertValueToPyObject(const gorgonzola::common::Value& value);

    py::object getAsDF();

    gorgonzola::pyarrow::Table getAsArrow(std::int64_t chunkSize, bool fallbackExtensionTypes);
...
private:
    static py::dict convertNodeIdToPyDict(const gorgonzola::common::nodeID_t& nodeId);

    void getNextArrowChunk(const std::vector<gorgonzola::common::LogicalType>& types,
        const std::vector<std::string>& names, py::list& batches, std::int64_t chunkSize,
        bool fallbackExtensionTypes);
    py::object getArrowChunks(const std::vector<gorgonzola::common::LogicalType>& types,

...
PyConnection::PyConnection(PyDatabase* pyDatabase, uint64_t numThreads) {
    storageDriver = std::make_unique<gorgonzola::main::StorageDriver>(pyDatabase->database.get());
        ...
        for (auto child : dict) {
            ...
            if (!LogicalTypeUtils::tryGetMaxLogicalType(childKeyType, curChildKeyType, resultKey)) {
                throw RuntimeException(stringFormat(
                    "Cannot convert Python object to Gorgonzola value : {}  is incompatible with {}",

...
public class GorgonzolaMap implements AutoCloseable {
     ...
     */
    public GorgonzolaMap(Value value) {
        mapVal = value;
    }
     ...
     */
    public GorgonzolaMap(Value[] keys, Value[] values) {
        ...
        mapVal = Native.gorgonzolaCreateMap(keys, values);
    }
    ...
    private Value getMapKeyOrValue(long index, boolean isKey) {
        ...
        Value structValue = Native.gorgonzolaValueGetListElement(mapVal, index);
        Value keyOrValue = new GorgonzolaList(structValue).getListElement(isKey ? 0 : 1);
        structValue.close();
        return keyOrValue;
    }
    ...
    public long getNumFields() {
        ...
        return Native.gorgonzolaValueGetListSize(mapVal);
    }

...
public class ValueTest extends TestBase {
    ...
    void CreateListLiteral() {
        ...
        GorgonzolaList list = new GorgonzolaList(listValues);
    ...
    void CreateListLiteralNested() {
        ...
        GorgonzolaList[] nestedLists = { new GorgonzolaList(nestedListValues[0]), new GorgonzolaList(nestedListValues[1]) };

        Value[] listValues = { nestedLists[0].getValue(), nestedLists[1].getValue() };
        GorgonzolaList list = new GorgonzolaList(listValues);
        assertEquals(listValues.length, list.getListSize());
        for (int i = 0; i < list.getListSize(); ++i) {
            GorgonzolaList nestedList = new GorgonzolaList(list.getListElement(i));
    ...
    void CreateListDefaultValues() {
        int listLength = 5;
        GorgonzolaList list = new GorgonzolaList(new DataType(DataTypeID.INT32), listLength);
    ...
    void ValueGetListSize() {
        ...
        assertFalse(value.isNull());
        GorgonzolaList list = new GorgonzolaList(value);
    ...
    void CreateStructLiteral() {
        ...
        GorgonzolaStruct structVal = new GorgonzolaStruct(fieldNames, fieldValues);
    ...
    void CreateStructLiteralNested() {
        ...
        GorgonzolaStruct person = new GorgonzolaStruct(personFieldNames, personFieldValues);

        String[] companyFieldNames = { "name", "boss" };
        Value[] companyFieldValues = { new Value("Company"), person.getValue() };
        GorgonzolaStruct company = new GorgonzolaStruct(companyFieldNames, companyFieldValues);

        GorgonzolaStruct actualPerson = new GorgonzolaStruct(company.getValueByFieldName("boss"));
    ...
    void CreateStructLiteralFromMap() {
        Map<String, Value> fields = Map.of(
                ...
                "ID", new Value(2),
                "age", new Value(20));
        GorgonzolaStruct structVal = new GorgonzolaStruct(fields);
    ...
    void StructValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        GorgonzolaStruct structVal = new GorgonzolaStruct(value);
    ...
    void CreateMapLiteral() {
        ...
        GorgonzolaMap gorgonzolaMap = new GorgonzolaMap(keys, values);
        assertEquals(keys.length, gorgonzolaMap.getNumFields());
        int aliceKeyIdx = (gorgonzolaMap.getKey(0).getValue().equals("Alice")) ? 0 : 1;
        int bobKeyIdx = 1 - aliceKeyIdx;
        assertEquals("Alice", gorgonzolaMap.getKey(aliceKeyIdx).getValue());
        assertEquals(1, (Integer) gorgonzolaMap.getValue(aliceKeyIdx).getValue());
        assertEquals("Bob", gorgonzolaMap.getKey(bobKeyIdx).getValue());
        assertEquals(2, (Integer) gorgonzolaMap.getValue(bobKeyIdx).getValue());

        PreparedStatement stmt = conn
                .prepare("MATCH (m:movies) WHERE m.name = 'Roma' SET m.audience = $audience");
        Map<String, Value> options = Map.of(
                "audience", gorgonzolaMap.getValue());
    ...
    void CreateMapLiteralNested() {
        ...
        GorgonzolaMap map0 = new GorgonzolaMap(nestedKeys[0], nestedValues[0]);
        GorgonzolaMap map1 = new GorgonzolaMap(nestedKeys[1], nestedValues[1]);
        Value[] nestedMaps = { map0.getValue(), map1.getValue() };

        Value[] keys = { new Value(Long.valueOf(0)), new Value(Long.valueOf(1)) };
        GorgonzolaMap gorgonzolaMap = new GorgonzolaMap(keys, nestedMaps);
        assertEquals(keys.length, gorgonzolaMap.getNumFields());

        GorgonzolaMap map1Actual = new GorgonzolaMap(gorgonzolaMap.getValue(1));
    ...
    void MapValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        GorgonzolaMap mapVal = new GorgonzolaMap(value);
        ...
        mapVal = new GorgonzolaMap(value);
        assertTrue(value.isOwnedByCPP());
        ...
        try (QueryResult result = conn
            ...
            try (FlatTuple flatTuple = result.getNext();
                ...
                try (Value nodeListValue = ValueRecursiveRelUtil.getNodeList(value)) {
                    ...
                    GorgonzolaList nodeList = new GorgonzolaList(nodeListValue);
                ...
                try (Value relListValue = ValueRecursiveRelUtil.getRelList(value)) {
                    ...
                    GorgonzolaList relList = new GorgonzolaList(relListValue);
